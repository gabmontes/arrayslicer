...
IndexedArray.prototype.fetch = function (value, orprev) {
    ...
    if (this.compare(value, this.maxv) === 1) {
        this.cursor = orprev && (this.data.length - 1) || null;
        return this;
    }
    ...
    if (pos) {
        ...
        cursor = pos.found ? pos.index : orprev && pos.prev || null;
    } else {
        // or do the actual search
        cursor = this.search.call(this, value);
        // return prev if not found and requested
        if (cursor === null && orprev)
            cursor = valpos[value].prev;
    }
...
IndexedArray.prototype.getRange = function (begin, end, aprox) {
    ...
    // start search
    var start = this.fetch(begin, aprox).cursor;
    var finish = this.fetch(end, aprox).cursor;
